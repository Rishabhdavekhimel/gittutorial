...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    console.error("Error in webhook:", error); // Log the error for debugging purposes
  ...
  if (event.type === "checkout.session.completed") {
    ...
    if (!session?.metadata?.userId) {
      return new NextResponse("User ID is missing in metadata. Unable to process subscription.", {
        status: 400,
      });
    }
